...
@pytest.fixture()
def asa_terminal_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> AsaTerminalClient:
    factory = algorand_client.client.get_typed_app_factory(
        AsaTerminalFactory, default_sender=deployer.address
    )
...
def test_says_hello(asa_terminal_client: AsaTerminalClient) -> None:
    result = asa_terminal_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    asa_terminal_client: AsaTerminalClient,
) -> None:
    result = (
        asa_terminal_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
